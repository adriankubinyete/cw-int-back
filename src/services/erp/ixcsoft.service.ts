...
import { type ErpAdapter, type ErpConfig, type Client, ClientStatus } from "./erp.interface";
...
export class IxcSoftService implements ErpAdapter {
    ...
    constructor(config: ErpConfig) {
        ...
        this.client.interceptors.response.use(
            (response) => {
                ...
                if (typeof data === "string" && data.includes("Ocorreu um erro ao processar")) {
                    serviceLogger.error("ixc returned html error page", {
                        method: response.config.method?.toUpperCase(),
                        url: response.config.url,
                        data,
                    });
                    throw new IxcApiError("Ocorreu um erro ao processar. Contate o suporte IXC Soft.");
                }
    ...
    async requestList<T>(path: string, body?: unknown): Promise<T[]> {
    ...
    async searchClientByPhone(phone: string): Promise<Client | null> {
        ...
        const registros = await this.requestList<any>("/cliente", {
    ...
    private mapClient(raw: any): Client {
        return {
            id: String(raw.id),
            name: raw.razao ?? raw.nome,
            document: raw.cnpj_cpf,
            phone: raw.telefone_celular ?? raw.fone_res,
            status: this.mapClientStatus(raw.ativo) ?? ClientStatus.UNKNOWN,
        };
    }
