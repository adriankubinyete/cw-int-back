...
export async function integrationsRoutes(app: FastifyInstance) {
    ...
    app.get('/integrations', async (request) => {
        ...
        routeLogger.debug('listing integrations', { userId: request.user.userId, erpType: query.erpType })
    ...
    app.get('/integrations/:id', async (request, reply) => {
        ...
        try {
            const integration = await integrationService.getById(request.user.userId, id)
            routeLogger.debug('getting integration', { userId: request.user.userId, integrationId: id })
            return integration
        } catch (error) {
    ...
    app.patch('/integrations/:id', async (request, reply) => {
        ...
        try {
            const integration = await integrationService.update(request.user.userId, id, body)
            routeLogger.info('integration updated', { userId: request.user.userId, integrationId: id })
            return integration
        } catch (error) {
            return reply.code(404).send({ message: 'Integration not found' })
        }
    ...
    app.post('/integrations/:id/test', async (request, reply) => {
        ...
        try {
            ...
            routeLogger.info('integration tested', { userId: request.user.userId, integrationId: id, success })

            if (success) {
                return { success: true }
            }

            return { success: false, error: 'Não foi possível conectar ao ERP. Verifique as credenciais e a URL.' }
        } catch (error) {
    ...
    app.delete('/integrations/:id', async (request, reply) => {
        ...
        try {
            ...
            routeLogger.info('integration deleted', { userId: request.user.userId, integrationId: id })
